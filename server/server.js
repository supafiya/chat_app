...
io.on('connection', (sock) => {
	...
	sock.on('nameChange', (name) => {
		...
		if (validation.identities(name) === false) {
			...
			let nameLC = name.trim();
			let reLCName = nameLC.toLowerCase();
			let reLCNameArray = users.getUserList(userOldRoom);
			...
			if (reLCNameArray.includes(reLCName)) {
				// add name validation here, auto change name to _# if exists.
